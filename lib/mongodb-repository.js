...
async function createBookingIfAvailable(booking) {
  return createBookingsIfAvailable([booking]);
}

async function createBookingsIfAvailable(records) {
  ...
  return withTransaction(async ({ bookings, bookingLocks }, session) => {
    for (const booking of records.slice().sort((a, b) => a.seatId.localeCompare(b.seatId))) {
      await bookingLocks.updateOne({ _id: booking.seatId }, { $inc: { revision: 1 } }, { upsert: true, session });
    ...
    await bookings.insertMany(records.map(booking => ({ ...booking })), { session });
    return true;
  });
}

module.exports = { initialize, getState, getBookings, getPayments, save, createBookingIfAvailable, createBookingsIfAvailable };
