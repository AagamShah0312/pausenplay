...
let state = Object.assign(defaultState(), repository.getState({}));
...
let bookings = repository.getBookings([]);
if (!Array.isArray(bookings)) bookings = [];
let payments = repository.getPayments([]);
...
function save() {
  ...
  saveTimer = setTimeout(() => {
    saveTimer = null;
    try {
      repository.save({ state, bookings, payments });
    } catch (err) {
...
module.exports = {
  ...
  STATE_FILE: repository.state,
  BOOKINGS_FILE: repository.bookings,
  PAYMENTS_FILE: repository.payments,
