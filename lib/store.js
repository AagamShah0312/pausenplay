 ...
 * Business rules and in-memory cache backed by the selected repository.
 */

const crypto = require('crypto');
const { getRepository } = require('./repository-selector');
const { DEFAULT_HOURLY_RATE_RUPEES, validateHourlyRate, getBookingPrice } = require('./pricing');
...
function normalizeState(saved) {
  ...
  state.seats.forEach(seat => { if (!validateHourlyRate(seat.hourlyRate).hourlyRate) seat.hourlyRate = DEFAULT_HOURLY_RATE_RUPEES; });
...
async function initialize() {
  ...
  if (typeof repository.initialize === 'function') await repository.initialize();
  normalizeState(await repository.getState(defaultState()));
  bookings = await repository.getBookings([]);
  payments = await repository.getPayments([]);
  if (!Array.isArray(bookings)) bookings = [];
  if (!Array.isArray(payments)) payments = [];
  initialized = true;
}

function requireInitialized() { if (!initialized) throw new Error('Store has not been initialized.'); }
...
/* --------------------------- persistence --------------------------- */
async function save() { requireInitialized(); await repository.save({ state, bookings, payments }); }
...
async function getPublicState() {
  ...
  const seats = state.seats.map(s => {
    ...
    return {
      ...
      booking: b ? {
        id: b.id,
        bookingGroupId: b.bookingGroupId || b.id,
        name: b.name,
        endAt: b.endAt,
        startAt: b.startAt,
      ...
      next: next ? { id: next.id, bookingGroupId: next.bookingGroupId || next.id, startAt: next.startAt, endAt: next.endAt, durationMin: next.durationMin } : null,
...
async function createBooking({ seatId, name, phone, minutes, startAt, date, time, createdBy = 'customer' }) {
  await sweepExpired(); // a session that just ran out must not block the station
  ...
  if (typeof repository.createBookingIfAvailable === 'function') {
    const reserved = await repository.createBookingIfAvailable(booking);
    if (!reserved) {
      return {
        error: check.scheduled
          ? `Station ${seat.label} is already booked at that time. Try another slot or station.`
          : `Station ${seat.label} was just booked by someone else. Pick another one.`
      };
    }
...
/** Create one or more station bookings as a single all-or-nothing group. */
async function createBookings({ stationIds, name, phone, minutes, startAt, date, time, createdBy = 'customer', bookingGroupId = uid() }) {
  if (!Array.isArray(stationIds) || !stationIds.length) return { error: 'Please pick at least one station.' };
  if (new Set(stationIds).size !== stationIds.length) return { error: 'A station can only be selected once.' };
  await sweepExpired();
  const checks = stationIds.map(seatId => validateBookingInput({ seatId, name, phone, minutes, startAt, date, time }));
  const invalid = checks.find(check => check.error);
  if (invalid) return { error: invalid.error };
  const t = now();
  const records = checks.map(check => ({
    id: uid(), bookingGroupId, seatId: check.seat.id, seatLabel: check.seat.label, zone: check.seat.zone,
    name: check.cleanName, phone: check.cleanPhone, startAt: check.start, endAt: check.start + check.minutes * 60000,
    durationMin: check.minutes, status: check.scheduled ? 'scheduled' : 'active', startedAt: check.scheduled ? null : t,
    endedAt: null, createdAt: t, createdBy, adjustments: []
  }));
  if (typeof repository.createBookingsIfAvailable === 'function') {
    const reserved = await repository.createBookingsIfAvailable(records);
    if (!reserved) return { error: 'One or more selected stations were just booked. Please choose another station or time.' };
    bookings.push(...records);
    return { bookings: records.map(publicBooking), bookingGroupId };
  }
  // JSON is test-only; prevalidation above preserves legacy behavior.
  for (const record of records) bookings.push(record);
  await save();
  return { bookings: records.map(publicBooking), bookingGroupId };
}

/** Store server-validated booking details until the Razorpay callback arrives. */
async function createPendingPayment({ razorpayOrderId, amountPaise, currency = 'INR', hourlyRate, booking }) {
  requireInitialized();
  ...
  const stationIds = booking && booking.stationIds;
  if (stationIds && (!Array.isArray(stationIds) || !stationIds.length || new Set(stationIds).size !== stationIds.length)) return { error: 'Please choose one or more unique stations.' };
  const check = validateBookingInput(booking || {});
  if (check.error) return { error: check.error };
  if (stationIds) {
    const stationChecks = stationIds.map(seatId => validateBookingInput({ ...booking, seatId }));
  ...
  const selectedSeats = stationIds ? stationIds.map(id => seatById(id)) : [check.seat];
  if (selectedSeats.some(seat => !seat)) return { error: 'Please pick a valid station.' };
  const calculated = selectedSeats.map(seat => getBookingPrice({ seatId: seat.id, minutes: check.minutes, hourlyRate: seat.hourlyRate }));
  if (calculated.some(price => price.error)) return { error: 'Unable to price this booking.' };
  const total = calculated.reduce((sum, price) => sum + price.amountPaise, 0);
  if (amountPaise !== total) return { error: 'Payment amount does not match this booking.' };
  ...
  const record = {
    ...
    booking: {
      ...
      minutes: check.minutes, startAt: check.start, stationIds: stationIds || [check.seat.id],
...
async function markPaymentFailed({ razorpayOrderId, razorpayPaymentId }) {
 ...
 */
async function finalizePaidBooking({ razorpayOrderId, razorpayPaymentId, amountPaise, currency = 'INR' }) {
  requireInitialized();
  const payment = paymentForOrder(razorpayOrderId);
  ...
  if (payment.bookingId) {
    if (payment.razorpayPaymentId === razorpayPaymentId) {
      ...
      const grouped = payment.bookingIds ? bookings.filter(b => payment.bookingIds.includes(b.id)) : booking ? [booking] : [];
      return booking ? { booking: publicBooking(booking), bookings: grouped.map(publicBooking), bookingGroupId: booking.bookingGroupId || booking.id, totalAmount: payment.amountPaise / 100, duplicate: true } : { error: 'Payment record is incomplete.' };
    }
  ...
  const result = await createBookings({ ...payment.booking, createdBy: 'customer' });
  if (result.error) {
  ...
  const rawBookings = result.bookings.map(item => bookings.find(b => b.id === item.id));
  rawBookings.forEach(rawBooking => {
    const rate = (seatById(rawBooking.seatId) || {}).hourlyRate;
    const price = getBookingPrice({ seatId: rawBooking.seatId, minutes: rawBooking.durationMin, hourlyRate: rate });
    rawBooking.payment = { status: 'paid', razorpayOrderId, razorpayPaymentId, amountPaise: price.amountPaise, currency, hourlyRate: rate, amount: price.amountPaise / 100, paidAt: now() };
  });
  payment.status = 'completed';
  payment.bookingId = rawBookings[0].id;
  payment.bookingIds = rawBookings.map(booking => booking.id);
  payment.razorpayPaymentId = razorpayPaymentId;
  await save();
  return { booking: publicBooking(rawBookings[0]), bookings: rawBookings.map(publicBooking), bookingGroupId: result.bookingGroupId, totalAmount: amountPaise / 100 };
}

/** Add or remove minutes from a live or reserved session (admin). */
async function adjustBooking(id, deltaMinutes, adminLabel = 'admin') {
...
async function endBooking(id, adminLabel = 'admin') {
  const b = bookings.find(x => x.id === id);
  if (!b) return { error: 'Booking not found.' };
  b.adjustments = b.adjustments || [];
  if (b.status === 'scheduled') {
...
async function startBooking(id, adminLabel = 'admin') {
  const b = bookings.find(x => x.id === id);
  if (!b) return { error: 'Booking not found.' };
...
async function listBookings({ includeActive = true, limit = 0 } = {}) {
  await sweepExpired();
  const list = bookings
...
async function updateStationPricing({ seatId, hourlyRate }) {
  const seat = seatById(seatId);
 ...
 */
async function updateLayout({ seats, layout }) {
  ...
  if (layout && typeof layout === 'object') {
    ...
    if (layout.imageFileId !== undefined) {
      const imageFileId = String(layout.imageFileId || '').trim();
      if (!/^[a-f\d]{24}$/i.test(imageFileId) && !/^memory-/.test(imageFileId)) return { error: 'That image reference is not allowed.' };
      state.layout.imageFileId = imageFileId;
      state.layout.imageContentType = String(layout.imageContentType || '').trim() || null;
    }
...
module.exports = {
  ...
  save,
  TZ,
  get DATA_DIR() { return repository && repository.DATA_DIR; },
  get STATE_FILE() { return repository && repository.state; },
  get BOOKINGS_FILE() { return repository && repository.bookings; },
  get PAYMENTS_FILE() { return repository && repository.payments; },
