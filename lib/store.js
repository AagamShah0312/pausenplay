...
async function finalizePaidBooking({ razorpayOrderId, razorpayPaymentId, amountPaise, currency = 'INR' }) {
  ...
  const result = await createBooking({ ...payment.booking, createdBy: 'customer' });
  if (result.error) {
...
async function updateLayout({ seats, layout }) {
  ...
  if (layout && typeof layout === 'object') {
    ...
    if (layout.imageKey !== undefined) {
      const imageKey = String(layout.imageKey || '').trim();
      if (!/^store-layout\/[a-zA-Z0-9._-]+$/.test(imageKey)) return { error: 'That image key is not allowed.' };
      state.layout.imageKey = imageKey;
