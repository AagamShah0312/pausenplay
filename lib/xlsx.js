...
/* ------------------------------- zip ------------------------------- */
/**
 * DOS date/time pair for a zip entry. Month/day of 0 (what you get from an
 * all-zero field) is not a valid DOS date and makes Windows/Excel treat the
 * archive as damaged, so every entry carries a real timestamp.
 */
...
function sheetXml(rows, opts = {}) {
  ...
  const lastRef = `${colName(Math.max(1, colCount) - 1)}${Math.max(1, rowCount)}`;
  ...
  /*
   * Excel is strict about the CT_Worksheet sequence (ECMA-376 Part 1):
   *   sheetPr, dimension, sheetViews, sheetFormatPr, cols, sheetData, … , autoFilter
   * Emitting <cols> before <sheetViews> makes Excel report unreadable content
   * and repair (or drop) the sheet, so the order below must not change.
   */
  out.push(`<dimension ref="A1:${lastRef}"/>`);
  out.push('<sheetViews><sheetView workbookViewId="0"' +
    (freezeHeader ? '><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/><selection pane="bottomLeft" activeCell="A2" sqref="A2"/></sheetView>' : '/>') +
    '</sheetViews>');
  ...
  out.push('</sheetData>');
  // autoFilter has to sit after sheetData — and only makes sense on data sheets
  if (opts.autoFilter && rowCount > 1 && colCount > 0) {
    out.push(`<autoFilter ref="A1:${colName(colCount - 1)}${rowCount}"/>`);
  }
  out.push('</worksheet>');
  return out.join('');
}
...
function corePropsXml(when) {
  const iso = when.toISOString().replace(/\.\d{3}Z$/, 'Z');
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
<dc:creator>PausenPlay</dc:creator>
<cp:lastModifiedBy>PausenPlay</cp:lastModifiedBy>
...
<Application>PausenPlay</Application>
</Properties>`;

/**
 * @param {Array<{name:string, rows:Array<Array>, cols?:number[], autoFilter?:boolean}>} sheets
...
function build(sheets) {
  ...
  files.push({ name: 'docProps/core.xml', data: corePropsXml(when) });
  files.push({ name: 'docProps/app.xml', data: APP_PROPS });
